...
public class PostMainFragment extends Fragment implements DialogPostAdapter.OnDialogListener {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        PostActivity.setTitle("Të dhënat kryesore");
        ...
        if(PostActivity.post.getNrDyer()!=0) {
            List<String> doors = Arrays.asList(getResources().getStringArray(R.array.dyer));
            tvDyer.setText(doors.get(PostActivity.post.getNrDyer()));
        } else {
            if(PostHomeFragment.clicked) {
        ...
        if(PostActivity.post.getDefekt()!=0) {
            List<String> defekt = Arrays.asList(getResources().getStringArray(R.array.opsione));
            tvDefekt.setText(defekt.get(PostActivity.post.getDefekt()));
        }

        if(PostActivity.post.getAksident()!=0){
            List<String> aksident = Arrays.asList(getResources().getStringArray(R.array.opsione));
            tvAksident.setText(aksident.get(PostActivity.post.getAksident()));
        }

        if(PostActivity.post.getTransmisioni()!=0) {
            List<String> transmisioni = Arrays.asList(getResources().getStringArray(R.array.transmisioni));
            tvTransmisioni.setText(transmisioni.get(PostActivity.post.getTransmisioni()));
        }
        else
        {
            if(PostHomeFragment.clicked) {
    ...
    public void onDialogItemClick(int position) {
        ...
        if(elementi==0)
        {
            ...
            PostActivity.post.setTitulli(dialogItems.get(position));
            tvModeli.setText(dialogItems.get(position));
            tvVarianti.setText("Përcakto variantin");
        }
        else if(elementi==2) {
            PostActivity.post.setKarburantiID(position + 1);
            PostActivity.post.setTitulli(PostActivity.post.getTitulli()+" "+dialogItems.get(position));
            ...
            PostActivity.post.setNrDyer(position);
            tvDyer.setText(dialogItems.get(position));
        }
        else if(elementi==8) {
            PostActivity.post.setDefekt(position);
            tvDefekt.setText(dialogItems.get(position));
        }
        else if(elementi==9) {
            PostActivity.post.setAksident(position);
            tvAksident.setText(dialogItems.get(position));
        }
        else if(elementi==10) {
            PostActivity.post.setTransmisioni(position);
