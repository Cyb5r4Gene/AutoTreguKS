...
public class PostTeknikeFragment extends Fragment implements DialogPostAdapter.OnDialogListener {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        ((PostActivity) getActivity()).getSupportActionBar().setTitle("Të dhënat teknike të veturës");
        ...
        radioMetalike.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(!tvPostNgjyra.getText().toString().trim().equals("Pa përcaktuar")) {
                    PostActivity.post.setNgjyra(tvPostNgjyra.getText().toString().trim()+", "+radioMetalike.getText().toString().trim());
                }
            }
        });
        ...
        radioMat.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(!tvPostNgjyra.getText().toString().trim().equals("Pa përcaktuar")) {
                    PostActivity.post.setNgjyra(tvPostNgjyra.getText().toString().trim()+", "+radioMat.getText().toString().trim());
                }
            }
        });
        ...
        if (PostActivity.post.getNgjyra() != null) {
            String ngjyra[] = PostActivity.post.getNgjyra().split(",",2);
            tvPostNgjyra.setText(ngjyra[0]);
            if(ngjyra[1].equals(" Metalike"))
                radioMetalike.setChecked(true);
            else
                radioMat.setChecked(true);
        }

        if (PostActivity.post.getMaterialiEnterier() != null)
            tvPostMateriali.setText(PostActivity.post.getMaterialiEnterier());

        if (PostActivity.post.getNgjyraEnterier() != null)
            tvPostNgjyraEnterier.setText(PostActivity.post.getNgjyraEnterier());
    ...
    public void onDialogItemClick(int position) {
        if (btnID == 1) {
            ...
            if (radioMat.isChecked())
                PostActivity.post.setNgjyra(dialogItems.get(position) + ", " + radioMat.getText());
            else
                PostActivity.post.setNgjyra(dialogItems.get(position) + ", " + radioMetalike.getText());
            tvPostNgjyra.setText(dialogItems.get(position));
        } else if (btnID == 7) {
            PostActivity.post.setMaterialiEnterier(dialogItems.get(position));
            tvPostMateriali.setText(dialogItems.get(position));
        } else if (btnID == 8) {
            PostActivity.post.setNgjyraEnterier(dialogItems.get(position));
